...
let queue = 0;
function changeSimFunction(specifiedSim = -1) {
    if (isChangingSim) {
...
function initNetwork(screen) {
    ...
    for (let i = 0; i < nodeQuantity; i++) {
        ...
        screen.objects.push(new node(nodeStartX, nodeStartY, nodeStartVelX, nodeStartVelY, nodeMaxLife, nodeLightColors[chosenDarkColor], nodeDarkColors[chosenLightColor], screen));
    }
}
